...
def combine_all_results():
    ...
    input_output = [("output_overlap_div_filtered.tsv", f"all_{output_dir}_overlap_div.tsv"), 
                    ("blatdiver_output.tsv", f"all_{output_dir}_blatdiver_output.tsv"),
                    ("no_gap_overlap_div.tsv", f"all_{output_dir}_no_gaps_overlap_div.tsv"),
                    ("no_gaps.tsv", f"all_{output_dir}_no_gaps.tsv"), 
                    ("overlap_and_gap.tsv", f"all_{output_dir}_overlap_and_gap.tsv"),
                    ("overlap.tsv", f"all_{output_dir}_overlap.tsv")]
...
def main():
    ...
    for record in SeqIO.parse(input_fasta, "fasta"):
        ...
        if seq_len > chunk_size:
            #CHANGED THIS SO THAT IT WILL REDO SOME OF THEM PAST 175
            for i in range(175*chunk_size, seq_len, chunk_size):
